  ...
  return [
    {
      ...
      designCheckpoints: ["执行前必须经过租户空间、业务团队和 Agent 模型白名单", "API Key 只保存 secret ref"],
    },
    {
      id: "agent",
      name: "Agent 定义层",
      objective: "定义 Agent 的角色、权限、工具集、提示词、记忆范围与团队归属。",
      backend: ["registry-core.ts", "tenant-space-core.ts", "db.ts"],
      frontend: ["agent-teams/page.tsx", "business-teams/page.tsx"],
      apiSurfaces: ["/api/task-runs/submit"],
      capabilities: ["团队 Leader 与成员 Agent", "在线展示角色、工具集、模型与记忆范围", "团队可见性治理"],
      extensibility: ["后续可增加 Agent 版本、灰度发布与审批流"],
      designCheckpoints: ["Agent 不能绕过所属团队运行约束", "Agent 团队必须绑定业务团队边界"],
    },
    {
      id: "tool-skill",
      name: "工具 / Skill 管理层",
      objective: "统一管理工具、skill、权限和审计，并把 skill 连接到 OpenViking 记忆。",
      backend: ["execution-policy-core.ts", "plugin-core.ts", "openviking-core.ts"],
      frontend: ["execution-policies/page.tsx", "knowledge/page.tsx", "settings/page.tsx"],
      ...
      designCheckpoints: ["权限与 Claude Code、运行约束模型对齐", "新增工具不能修改任务主流程"],
    },
    {
      id: "orchestration",
      name: "多 Agent 编排层",
      ...
      frontend: ["task-runs/[id]/page.tsx", "task-run-ops-console.tsx"],
      apiSurfaces: ["/api/task-runs/[id]/tick", "/api/task-runs/[id]/nodes/[nodeId]/retry"],
      capabilities: ["规则计划与 DAG 计划", "任务模板导入", "节点依赖解锁", "失败节点独立重试"],
      extensibility: ["可从线性流程扩展到 Leader 生成 DAG"],
      designCheckpoints: ["节点执行必须记录 trace/event", "跨团队调用必须先过跨团队授权"],
    },
    {
      id: "team-execution",
      name: "Agent 团队任务执行层",
      ...
      frontend: ["task-runs/page.tsx", "task-runs/[id]/page.tsx", "trace-group.tsx"],
      apiSurfaces: ["/api/task-runs/[id]/execution-board", "/api/task-run-interventions/[id]/resolve"],
    ...
    },
    {
      id: "biz-team",
      name: "业务团队管理层",
      objective: "用租户空间和业务团队支撑多团队分权、多可见性和跨团队服务调用。",
      backend: ["tenant-space-core.ts", "access-grant-core.ts", "registry-core.ts"],
      frontend: ["tenant-spaces/page.tsx", "business-teams/page.tsx", "access-grants/page.tsx"],
      apiSurfaces: ["/api/task-runs/submit"],
      capabilities: ["租户空间和业务团队隔离", "创建者/编辑者/使用者预留", "个人/团队/全局可见性"],
      extensibility: ["可扩展 ACL 表、审批链和组织目录同步"],
      designCheckpoints: ["任务全局可见但权限动作受控", "跨业务团队调用走跨团队授权"],
    },
    {
      id: "task-board",
      name: "任务执行展示层",
      ...
      frontend: ["page.tsx", "task-runs/page.tsx", "wallboard/page.tsx"],
      apiSurfaces: ["/api/task-runs/[id]/costs", "/api/task-runs/[id]/policy-hits"],
      capabilities: ["一次性、定时、Webhook、跨团队授权任务统计", "团队看板", "优先级与成本展示"],
  ...
  return [
    {
      ...
      trigger: "webhook",
      steps: ["Webhook 接收 MR diff", "生成可观测任务", "检视 Agent 团队按 skill 分层检查", "汇总评论并回写 MR", "反馈写回 OpenViking"],
