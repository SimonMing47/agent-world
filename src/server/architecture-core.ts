...
export function getArchitectureLayers(): ArchitectureLayer[] {
  return [
    ...
    },
    {
      ...
      backend: ["environment-core.ts", "db.ts", "api/webhooks/[pathKey]/route.ts"],
      frontend: ["settings/page.tsx", "runtimes/page.tsx"],
      apiSurfaces: ["/api/webhooks/[pathKey]", "/api/runtimes/discover"],
      capabilities: ["代码仓环境配置", "执行人和 PRIVATE_KEY 引用", "沙箱配置预留"],
  ...
  return [
    {
      ...
      configuration: ["任务模板 template-shield-mr-review", "环境 env-shield-mr-review", "团队 PR Vanguard"],
    ...
    },
    {
      ...
      configuration: ["任务模板 template-daily-security-review", "环境 env-daily-security-scan", "通知插件 builtin.notify.email"],
