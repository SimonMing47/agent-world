...
Initial sign-in uses the bootstrap administrator account. By default the username is `admin` and the password is `admin`; update the password when prompted after sign-in. For deployment, set the `AGENTWORLD_BOOTSTRAP_*` variables before the first start if you need a different initial account.
...
| `AGENTWORLD_BOOTSTRAP_USERNAME` | Initial local administrator username. Defaults to `admin`. |
| `AGENTWORLD_BOOTSTRAP_PASSWORD` | Initial local administrator password. Defaults to `admin`. |
| `AGENTWORLD_BOOTSTRAP_EMAIL` | Initial local administrator email. Defaults to `admin@agentworld.local`. |
| `AGENTWORLD_BOOTSTRAP_NAME` | Initial local administrator display name. |
| `AGENTWORLD_BOOTSTRAP_TITLE` | Initial local administrator title. |
