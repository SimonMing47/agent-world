...
- OpenViking runtime is installed by the managed CLI; deployment bundles may also provide a server binary
...
The installer clones the repository into `~/.agentworld/agent-world`, installs dependencies, bootstraps `.env.local` and SQLite, installs/prepares OpenViking, builds the production app, and creates an `agentworld` command in `~/.local/bin`.
...
| `agentworld install` | Install dependencies, bootstrap local config and SQLite, install OpenViking, and build the production app. |
...
`agentworld install` installs a managed OpenViking runtime when no bundled server binary is available. To repair or refresh that runtime manually:
...
5. `.venv-openviking/bin/openviking-server` from the managed source install.
