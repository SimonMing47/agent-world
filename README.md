...
- Provider 执行层：默认内置 Pi Runtime Adapter；Hermes、LangGraph、Mastra、Claude Code、OpenClaw 和企业自研运行时通过 Adapter / 插件扩展。
...
- Pi Runtime Adapter 基线；`src/server/runtime-adapter-core.ts` 现在提供统一 `AgentRuntimeInterface`，把健康检查、会话启动、恢复、Team 调用、事件流与 artifact 收口到 Pi 适配层；Hermes、LangGraph、Mastra、OpenClaw、自定义运行时后续按同一接口扩展。
- Provider Profile 支持 OpenAI Compatible、OpenAI Responses、OpenAI Chat / Completions、Anthropic、Azure OpenAI 等接口风格，并通过配置映射到 Pi 的统一模型抽象。
- Agent 定义中心：`/agents`，支持按默认系统提示词定义 Agent，配置默认 Provider / Runtime / 模型 / 工具 / 记忆范围，并附带 Agent Harness 配置，包括审批模式、Thinking 强度、人工介入、允许 / 禁止工具、仓库权限、记忆权限和 Secret 权限。
...
- 交互实验台：`/interactions`、`/interactions/:id`，用于真实模型对话、Team 会话、thinking / tool call 轨迹和人工介入；它保留为 runtime / human-in-the-loop 实验面，不再承担任务内核建模职责。单 Agent 会话可直接选择 Agent 定义并继承默认系统提示词与 Harness 画像。
