...
- 执行过程完整可追溯：任务空间必须记录 plan、reasoning summary、tool use、tool result、approval、retry、cost、policy hit、Finding 和 artifact。
- 记忆可沉淀、可读取、可反馈：OpenViking 作为分层记忆服务，提供 Skill、上下文和人工反馈的长期存储。
- 部署简单：应用发布包面向 Linux 自包含运行，OpenViking 通过服务端二进制直连，不依赖容器运行时。

## 3. 平台能力域

1. Provider 执行层：默认内置 Pi Runtime Adapter；Hermes、LangGraph、Mastra、Claude Code、OpenClaw 等运行时通过插件接入。
...
7. 任务执行展示层：按业务团队、任务蓝图、触发方式、任务类别、状态、Finding、成本和优先级组织任务看板。
