...
AgentWorld 定位为团队级 Agent 平台。平台核心不是单人聊天，也不是固定流程自动化，而是将 Provider 执行、Agent 定义、工具与知识、Agent 团队编排、任务执行、业务团队治理、执行展示、执行环境和记忆管理统一为一个可治理系统。
...
- 记忆可沉淀、可读取、可反馈：AgentWorld 知识引擎 作为分层记忆服务，提供可加载知识、上下文和人工反馈的长期存储。
...
3. 工具 / 知识管理层：管理工具和可加载知识，权限模型采用 allow / deny / approval，并与运行约束对齐。
...
9. 记忆层：基于 AgentWorld 知识引擎 做分层、分域、分团队的记忆和知识管理。
...
- 记忆：展示 AgentWorld 知识引擎 层级、可加载知识、知识条目和 L0/L1/L2 读取。
