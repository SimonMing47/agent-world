...
记忆与 Skill 层负责为 AgentWorld 提供长期上下文、可复用能力、执行经验和人工反馈沉淀。默认记忆服务为 AgentWorld 知识引擎，平台通过 `agentworld://knowledge/` URI 组织资源、Agent Skill 和用户记忆空间。
...
agentworld://knowledge/agent/skills/agentworld/...
...
- `agentworld://knowledge/agent/skills/agentworld/...`：Agent 可复用 Skill，包括检查方法、输出规范、领域提示和示例。
...
metadata:
  ...
  version: 1.0.0
spec:
  uri: agentworld://knowledge/agent/skills/agentworld/code-inspection/security
...
Skill 必须声明输入输出契约、适用领域、Finding 分类和版本。Skill 内容可以存储在 AgentWorld 知识引擎，平台注册表保存索引和权限元数据。
