...
- 支持全局知识、领域知识、代码仓知识、Agent Team 可加载知识和个人记忆统一检索。
...
- `project`：项目或代码仓上下文空间。通过 `project_key`、仓库或任务输入匹配。
- `agent_team`：Agent Team 专属知识、方法论和输出规范。

建议新增类型：

- `personal`：个人知识或用户记忆。默认私有，只能由创建人和显式授权的 Agent、Agent Team 或 Task Blueprint 读取。

知识空间同时有一个独立的知识范围字段 `knowledgeCategory`：
...
- Agent 可加载知识改进：写入 agent_team 或 agent/knowledge 空间。
...
- requiredSpaces、knowledgeSpaces、archiveOutputTo；旧 `skillSpaces` 仅作为兼容字段读取。
- 提交前的知识访问预览。
...
- 支持任务总结、人工反馈、Agent 可加载知识改进写回不同空间。
