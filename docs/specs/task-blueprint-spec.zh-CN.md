...
- 将 Finding、事件流、人工处理、成本、重试和可靠性状态纳入一等模型。
...
- 进行工具调用、Provider 流式输出、结果解析和成本记录。
- 在工具或写操作触发 ask 时暂停并等待人工处理。
- 写入 invocation、tool、permission、finding、artifact、cost 等事件。
