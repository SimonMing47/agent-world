...
ProviderAdapter 是 AgentWorld 中 Agent 调用层的标准适配接口。它屏蔽内置 Agent Runtime、Hermes、LangGraph、Mastra、Claude Code、OpenClaw、HTTP 模型服务、企业内部 CLI 或其他执行器的差异，使调度层只面对能力、状态和事件，而不直接依赖具体 Provider 实现。

ProviderAdapter 不负责创建任务、修改调度状态或决定业务流程。它只执行调度器分派的 Agent 调用节点。

## 2. 设计目标

- 将 Provider 选择、模型调用、工具协议、流式输出、取消和重试分类收敛到统一接口。
...
taskRunRef:
  id: run_001
nodeRef:
  id: node_inspection_security
agentRef:
  id: agent_security_inspector
...
- Adapter 不得无限内部重试；重试次数由 TaskBlueprint 和调度器控制。
...
- Adapter 输出可被任务事件流、Trace Span、看板和运行统计统一消费。
