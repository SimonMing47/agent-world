...
- 支持 single、sequential、parallel、dag、review-gate 等编排模式。
- 使 Leader、成员 Agent、评审 Agent 和汇总 Agent 的职责可配置。
...
spec:
  leader:
    agentRef: agent-review-leader
  members:
    - role: security-reviewer
      agentRef: agent-security-reviewer
    - role: test-reviewer
      agentRef: agent-test-reviewer
  orchestration:
    mode: dag
    planner: static
    nodes: []
  inputContractRef: schema-code-review-input
  outputContractRef: schema-code-review-output
  service:
    visibility: team
    sla:
      timeoutSeconds: 3600
