...
Server 插件当前支持 `authAdapters`、`repositoryConnectors`、`webhookParsers`、`toolBundles` 和 `outputPublishers`。其中 OIDC SSO 的 state、nonce、PKCE、token exchange、JWKS 校验、用户入库和 session cookie 由核心平台托管；插件只贡献 `authAdapters` 声明和配置 schema。SSO 插件开发细节见 `docs/specs/sso-plugin-development.zh-CN.md`。
  ...
  authAdapters: [
    {
      ...
      mode: "redirect",
      protocol: "oidc",
      capabilities: ["authorization_code_flow", "pkce", "jwks_verify", "claim_mapping"]
    }
  ],
...
- 管理员上传 `.awp` 或提交 `agentworld.plugin.json` 后，`GET /api/plugins/manifests` 能看到插件 manifest、扩展点、contributions 和 runtimeRegistry。
- SSO 插件的 `authAdapters` 能出现在身份与访问的 Adapter 下拉框。
- OIDC SSO 登录走 `/api/auth/plugins/:adapterId/start` 与 `/api/auth/plugins/:adapterId/callback`，成功后由核心写入 session cookie。
