...
- 允许插件贡献 ProviderAdapter、工具、知识资产、TaskBlueprint、Environment Template、Webhook 和看板视图。
...
- `authAdapters`：SSO 登录入口、协议声明、claim mapping 和身份标准化能力。
- `providerAdapters`：ProviderAdapter manifest 和运行时实现。
- `repositoryConnectors`：代码仓库、合并请求、文件读取和比较能力。
- `webhookParsers`：Webhook 验签、payload 解析和幂等键生成。
- `outputPublishers`：评论、通知、归档和外部发布能力。
- `toolBundles`：由平台代理执行或由插件执行的工具动作。
- `knowledgeAssets`：Agent 可加载知识定义、AgentWorld 知识引擎 URI 绑定和版本信息；历史 `skills` 字段作为兼容别名读取。
- `taskBlueprints`：可导入 Task Blueprint。
- `environmentTemplates`：可导入 Environment Template。
- `workflowBlocks`：可在任务蓝图中选择的通用编排阶段。
- `navigationItems`、`settingsPanels`、`dashboardWidgets`、`taskRunPanels`、`agentDetailTabs`：受控 UI 插槽。
- `codebaseEngines`：代码索引、图谱、增量同步和查询能力。
- `knowledgeSources`：外部知识导入、同步和检索能力。
- `secretProviders`：secret ref 解析能力。
...
`resolveSecretRef` 只接受来自插件配置、知识配置或页面配置的密钥值，不支持 `env:` 环境变量引用。

当前 Server Runtime 已落地的可执行贡献项为 `authAdapters`、`repositoryConnectors`、`webhookParsers`、`outputPublishers` 和 `toolBundles`。OIDC SSO 的安全链路由核心托管，插件贡献 `authAdapters` 后通过 `/api/auth/plugins/:adapterId/start` 与 `/api/auth/plugins/:adapterId/callback` 接入。其他贡献项已在 manifest 和扩展点中预留，后续应按同一 registry 与权限模型接入。
