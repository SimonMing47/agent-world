...
本文说明如何把一个外部代码仓库平台接入 AgentWorld，并让任务编排、Agent 可加载知识、知识库和反馈闭环组合出代码变更检视流程。平台可以是 GitHub、Gitea、CodeHub 或企业内部系统；AgentWorld 核心只依赖通用插件能力，不内置任何专用业务流程。
...
- 知识负责检视规则：关注维度、判断规则、意见模板、误报/命中经验、仓库专有知识。
- 任务蓝图负责编排：选择 Agent 团队、阶段顺序、权限、发布渠道和反馈策略。
- 知识库负责持久化：可加载知识、代码仓知识、通用领域知识、反馈沉淀都应可检索。

不要把某个具体业务名、报告名、外部平台名写进通用执行内核。需要差异化时，通过插件 manifest、任务蓝图配置、Agent 绑定知识和知识库内容表达。
...
3. `toolBundle`
   ...
   暴露可被任务节点调用的工具，例如 `pull_request.files`、`pull_request.rule_scan`。规则扫描工具应从节点加载的知识规则读取规则，并把结果写成通用 Finding。
...
Agent 可加载知识写入知识库后，Agent 通过 `knowledge:<knowledgeId>`、知识 URI 或能力画像 `knowledgeRefs` 绑定。任务运行时会加载该 Agent 的知识，自动把对应知识 URI 加进检索范围，并把知识中的 `heuristics.rules` 注入插件工具；旧 `skillRefs` 仅作为历史兼容字段读取。
...
1. 上下文加载阶段
   ...
   使用 `memory.retrieve` 读取 `codebase`、`domain`、`global` 类知识，范围限制到当前仓库和当前任务需要的知识。

2. 分层扫描阶段

   多个 `plugin_tool` 节点分别绑定不同 Agent。每个 Agent 只加载自己的知识，调用同一个平台插件扫描工具，产出通用 Finding。
...
- 认证：通过 `tokenRef` 和 `webhookSecretRef` 从插件配置、知识配置或页面配置读取密钥；不得使用 AgentWorld 环境变量承载代码平台 token 或 webhook secret，也不把 token 写进蓝图明文。
...
- 每个扫描 Agent 的事件里能看到已加载的知识和对应知识 URI。
- 插件工具创建的 Finding 带有文件、行号、规则 ID、commit SHA 和知识引用。
- 外部 MR/issue 下出现逐条 Finding 评论和汇总评论。
- 评论中的反馈链接无需登录可访问。
- 提交准确/不准确反馈后，`inspection_feedback` 有记录，代码仓知识空间出现对应反馈知识条目。
- 后续同仓库任务能按 `codebase` 类知识和仓库名检索到反馈沉淀。
